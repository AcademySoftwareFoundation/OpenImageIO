...
private:
    ...
    inline bool writefmt(const Str& fmt, Args&&... args)
    {
        ...
        bool ok       = m_io->write(s) == s.size();
        if (!ok)
            errorfmt("RGBE write error");
        return ok;
    }
};
    ...
    while (cur < numbytes) {
        ...
        while (cur < beg_run) {
            ...
            if (nonrun_count > 128)
                nonrun_count = 128;
            buf[0] = nonrun_count;
            if (m_io->write(buf, 1) < 1) {
                errorfmt("RGBE write error");
                return false;
            }
            if (m_io->write(&data[cur], nonrun_count) < size_t(nonrun_count)) {
                errorfmt("RGBE write error");
                return false;
            }
            cur += nonrun_count;
...
HdrOutput::RGBE_WritePixels(float* data, int64_t numpixels)
{
    ...
    if (m_io->write(rgbe, 4 * numpixels) < 4 * size_t(numpixels)) {
        errorfmt("RGBE write error");
        return false;
    }
    return true;
