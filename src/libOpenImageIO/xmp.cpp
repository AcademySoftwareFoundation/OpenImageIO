...
// Bounds on the work one decode_xmp() call may do. The XMP found in real
// files is a few hundred bytes to a few tens of KB, nested a handful of
// levels deep, holding tens of attributes. These caps sit far above anything
// legitimate; they exist because every one of those quantities is otherwise
// bounded only by the length of an attacker-supplied packet.
...
// Running totals for one decode_xmp() call. `bytes` accumulates the size
// add_attrib() reports for each attribute it writes; for a list attribute
// that size is the whole re-joined list, so this total is the quadratic cost
// of growing the list, and capping it caps that cost directly.
struct XMPbudget {
    size_t bytes   = 0;
    size_t attribs = 0;
