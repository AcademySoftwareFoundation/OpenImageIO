                 ...
                 OpenMode mode)
{
    ...
    if (m_io) {
        m_io->seek(0);
        m_tif = TIFFClientOpen(name.c_str(), openmode, m_io, writer_readproc,
                               writer_writeproc, writer_seekproc,
                               writer_closeproc, writer_sizeproc,
                               writer_mapproc, writer_unmapproc);
    } else {
#ifdef _WIN32
