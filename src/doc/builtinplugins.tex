...
\product's support of Cineon is preliminary.  Currently \product does not
write Cineon files at all.
...
\qkw{BitsPerSample} & int & bits per sample \\
...
\qkw{BitsPerSample} & int & the true bits per sample of the DPX file. \\
...
\hline
\qkw{BitsPerSample} & int & the true bits per sample in the ICO file. \\
...
\qkw{updirection} & string & set to \qkw{y} for OpenEXR
  environment maps, to indicate OpenEXR's convention that the ``up''
  direction is $+y$. \\
...
\ImageSpec Attribute & Type & PNG header data or explanation \\
\hline
\qkw{BitsPerSample} & int & the true bits per sample of the file
...
\qkw{BitsPerSample} & int & the true bits per sample in the PIC file.
\end{tabular}
...
\qkw{PixelAspectRatio} & float & pixel aspect ratio \\
\qkw{BitsPerSample} & int & the true (in the file) bits per sample. \\
\end{tabular}
\\ 
\vspace{.25in}
...
\qkw{BitsPerSample} & int & The actual bits per sample in the file (may
...
\item Only multiples of 8 bits per pixel may be passed through
  ...
  etc.  But the \qkw{BitsPerSample} attribute in the \ImageSpec
