...
\apiitem{int {\ce pixels} () const}
Retrieve the total number of pixels.
\apiend

\apiitem{int {\ce channels} () const}
Retrieve the number of channels.
\apiend

\apiitem{int {\ce A_channel} () const \\
int {\ce AR_channel} () const \\
int {\ce AG_channel} () const \\
int {\ce AB_channel} () const \\
int {\ce Z_channel} () const \\
int {\ce Zback_channel} () const}
\NEW % 1.8
...
\apiitem{int {\ce alpha_channel_for} (int c) const}
\NEW % 1.8
Return the channel number that is the alpha channel for channel {\cf c}.
(For example, the alpha channel for the \qkw{R} channel will be {\cf AR_channel()}.)
If there is no sensible alpha channel for channel {\cf c}, -1 will be
returned.
\apiend
...
\apiitem{int {\ce cull_behind} (int pixel, float depth)}
\NEW % 1.8
Remove all samples that lie completely behind {\cf depth}, returning the
