...
following values:
\begin{description}
\item[\halfspc \rm \kw{ImageIOFormatSpec::UnknownLinearity}] the default,
  indicates that you have made no claim about the color space of your
  pixel data.
\item[\halfspc \rm \kw{ImageIOFormatSpec::Linear}] indicates that the pixel
  values you are pssing repesent linear values.
\item[\halfspc \rm \kw{ImageIOFormatSpec::GammaCorrected}] indicates that the
  ...
  \ImageIOFormatSpec.
\item[\halfspc \rm \kw{ImageIOFormatSpec::sRGB}] indicates that the color pixel
...
JPEG/JFIF file, the values must be stored as 8-bit unsigned integers.
