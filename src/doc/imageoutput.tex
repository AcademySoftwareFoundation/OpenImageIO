...
\begin{code}
        ...
        ImageSpec spec (xres, yres, channels, PT_UINT8);
...
\begin{code}
        ...
        out->write_rectangle (xmin, xmax, ymin, ymax, zmin, zmax, PT_UINT8, rect);
        ...
\end{code}
...
\begin{code}
        ImageSpec spec (xres, yres, channels, PT_UINT16);
\end{code}
...
\item Write only a subset of channels to disk.  In this example, our
  ...
  \begin{code}
        ...
        ImageOutput *out = ImageOutput::create (filename);
        ImageSpec spec (xres, yres, 1, PT_UINT8);
        out->open (filename, spec);
...
\begin{code}
        ...
        ...
        ImageOutput *out = ImageOutput::create (filename);
        ImageSpec spec (cropwidth, croplength, channels, PT_UINT8);
...
\begin{code}
        int channels = 4;
        ImageSpec spec (width, length, channels, PT_UINT8);
...
\begin{code}
        ImageSpec spec (width, length, channels, PT_UINT16);
        spec.quant_black  = 0;
...
\apiitem{bool {\ce write_scanline} (int y, int z, ParamBaseType format,
     const void *data, \\
...
\apiitem{bool {\ce write_tile} (int x, int y, int z, ParamBaseType format,
...
\apiitem{bool {\ce write_image} (ParamBaseType format, const void *data, \\
