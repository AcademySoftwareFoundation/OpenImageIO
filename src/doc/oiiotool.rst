...
.. option:: --thumbnail-get
    ...
    `:fail=` *int* (default: 1)
      ...
      If 0, one is generated by downsizing the image to fit within a 256x256
      box, preserving aspect ratio and never enlarging, so a 512x384 image
      yields a 256x192 thumbnail. Generating reads the image's pixels;
      retrieving an embedded thumbnail does not.
