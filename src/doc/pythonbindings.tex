...
\apiitem{DeepData.{\ce init} (npixels, nchannels, pixeltypes)}
...
described by {\cf pixeltypes}, a tuple of \TypeDesc values (one per
channel). After calling {\cf init}, you still need to set the number of
...
float DeepData.{\ce deep_value} (pixel, channel, sample)}
Set or retrieve the specific float value of a given pixel, channel, and
sample index.
\apiend

\apiitem{DeepData.{\ce set_deep_value_uint} (pixel, channel, sample, value) \\
int DeepData.{\ce deep_value_uint} (pixel, channel, sample)}
Set or retrieve the specific unsigned integer value of a given pixel,
channel, and sample index.
\apiend

\apiitem{DeepData.{\ce npixels}}
This {\cf int} field constains the total number of pixels in this
collection of deep data.
\apiend

\apiitem{DeepData.{\ce nchannels}}
...
bool ImageOutput.{\ce write_deep_tiles} (xbegin, xend, ybegin, yend, zbegin, zend, deepdata) \\
...
not exist or does not have a type that matches {\cf typedesc}.

\noindent Example:
\begin{code}
    formats = oiio.getattribute ("format_list", oiio.TypeDesc.TypeString)
\end{code}
\apiend
