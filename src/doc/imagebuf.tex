...
\apiitem{ImageBuf (const std::string \&name, int subimage=0, int miplevel=0, \\
...
\apiitem{void reset (const std::string \&name, int subimage=0, int miplevel=0, \\
\bigspc\bigspc ImageCache *imagecache = NULL)}
Destroys any previous contents of the \ImageBuf and re-initializes it
...
\apiitem{bool write (const std::string \&filename, \\
...
\apiitem{void interppixel (float x, float y, float *pixel, \\
  \bigspc                            WrapMode wrap=WrapBlack) const}
Sample the image plane at coordinates $(x,y)$,
using linear interpolation between pixels, placing the result in
{\cf pixel[0..n-1]}, where $n$ is the smaller of {\cf maxchannels}
or the actual number of channels stored in the buffer.  It is up to
the application to ensure that {\cf pixel} points to enough memory
to hold the required number of channels. Note that pixel data values
themselves are at the pixel centers, so pixel $(i,j)$ is at image plane
coordinate $(i+0.5, j+0.5)$.
...
\apiitem{void interppixel_NDC_full (float s, float t, float *pixel, \\
  \bigspc                            WrapMode wrap=WrapBlack) const}
Given 2D floating point normalized image-space coordinates ({\cf s, t}),
linearly interpolate the surrounding pixels to yield interpolated value
{\cf pixel[0..n-1]}, where $n$ is the smaller of {\cf maxchannels}
or the actual number of channels stored in the buffer.  It is up to
the application to ensure that {\cf pixel} points to enough memory
to hold the required number of channels.  The coordinates {\cf (s,t)}
are in \emph{normalized image space}, where (0,0) is the upper left corner
