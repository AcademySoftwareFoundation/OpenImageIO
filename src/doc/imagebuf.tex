...
All I/O involving \ImageBuf (that is, calls to {\cf read}, {\cf write},
and {\cf save}) are implemented in terms of \ImageCache, \ImageInput,
...
\apiitem{ImageBuf (const std::string \&name, const ImageSpec \&spec)}
...
\apiitem{void reset (const std::string \&name, const ImageSpec \&spec)}
...
\apiitem{ImageBuf (const std::string \&name, ImageCache *imagecache=NULL)}
Construct an \ImageBuf that will be used to read the named file.  But
don't read it yet!  If {\cf imagecache} is non-NULL, the custom
...
\apiitem{void reset (const std::string \&name, ImageCache *imagecache = NULL)}
Destroys any previous contents of the \ImageBuf and re-initializes it
...
Reads the particular subimage and MIP level of the image.  Generally,
...
\apiitem{bool init_spec (const std::string \&filename,
...
resolution or data type.
\apiend


\subsection*{Saving an \ImageBuf to a file}

\apiitem{bool save (const std::string \&filename = std::string(), \\
  \bigspc               const std::string \&fileformat = std::string(), \\
  \bigspc               ProgressCallback progress_callback=NULL, \\
  \bigspc               void *progress_callback_data=NULL) const}
Save the image to the named file (an empty filename means use the original
filename given when the ImageBuf was created or reset) and file
format (an empty format means to infer the type from the filename
...
\apiitem{const std::string \& name (void) const}
Returns the name of the buffer (name of the file, for an
\ImageBuf read from disk).
\apiend

\apiitem{const std::string \& file_format_name (void) const}
...
\apiitem{void set_full (int xbegin, int xend, int ybegin, int yend, \\
  \bigspc\spc       int zbegin, int zend, const float *bordercolor=NULL)}
...
\apiitem{void interppixel_NDC_full (float s, float t, float *pixel, \\
  \bigspc                            WrapMode wrap=WrapBlack) const}
Given 2D floating point image-space coordinates ({\cf s, t}), linearly
interpolate the surrounding pixels to yield interpolated value
{\cf pixel[0..n-1]}, where $n$ is the smaller of {\cf maxchannels}
or the actual number of channels stored in the buffer.  It is up to
the application to ensure that {\cf pixel} points to enough memory
to hold the required number of channels.  The coordinates {\cf (s,t)}
are in \emph{image space}, where (0,0) is the upper left corner
of the full (a.k.a. ``display'') window and (1,1) is the lower right
cornell of the full/display window.
