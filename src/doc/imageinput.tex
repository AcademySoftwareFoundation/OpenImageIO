...
The \ImageIOFormatSpec has a field that reveals what color space the
...
following values:
\begin{description}
\item[\halfspc \rm \kw{ImageIOFormatSpec::UnknownLinearity}] indicates
  indicates it's unkown what color space the image file is using.
\item[\halfspc \rm \kw{ImageIOFormatSpec::Linear}] indicates that the
  color pixel values are known to be linear.
\item[\halfspc \rm \kw{ImageIOFormatSpec::GammaCorrected}] indicates
  ...
  \ImageIOFormatSpec.
\item[\halfspc \rm \kw{ImageIOFormatSpec::sRGB}] indicates that the
