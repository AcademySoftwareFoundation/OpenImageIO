...
\item Search for an ImageIO plugin that is capable of reading the file
  ...
  The {\cf open()} method returns a {\cf ImageInput::unique_ptr} that will
...
\begin{code}
        ImageInput::unique_ptr in = ImageInput::open ("myfile.tif");
        const ImageSpec &spec = in->spec();
        ...
...
\apiitem{ImageInput::unique_ptr {\ce open} (const std::string \&filename, \\
...
\apiitem{ImageInput::unique_ptr {\ce create} (const std::string \&filename, \\
