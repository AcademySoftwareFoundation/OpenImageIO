...
1. Read the base class definition from {file}`imageio.h`. It may also be
...
2. Declare these public items:
   ...
   > 1. An integer called `name_imageio_version` that identifies
   ...
   > 2. An function named `name_imageio_library_version` that identifies
   ...
   > 3. A function named `name_input_imageio_create` that
   ...
   > 4. An array of `char *` called `name_input_extensions`
...
3. The definition and implementation of an ImageInput subclass for this file
   ...
   1. `format_name()` should return the name of the format, which ought to
      match the name of the plugin and by convention is strictly lower-case
      and contains no whitespace.
   2. `open()` should open the file and return true, or should return false
   ...
   4. `read_native_scanline()` should read a single scanline from the file
      into the address provided, uncompressing it but keeping it in its naive
      data format without any translation.
   5. The virtual destructor, which should `close()` if the file is still
   ...
   06. `supports()`, only if your format supports any of the optional
       features described in the section describing `ImageInput::supports`.
   07. `valid_file()`, if your format has a way to determine if a file is
       of the given format in a way that is less expensive than a full
       `open()`.
   08. `seek_subimage()`, only if your format supports reading multiple
       subimages within a single file.
   09. `read_native_scanlines()`, only if your format has a speed advantage
   ...
   10. `read_native_tile()`, only if your format supports reading tiled
       images.
   11. `read_native_tiles()`, only if your format supports reading tiled
   ...
   12. ``` Channel subset'' versions of ``read_native_scanlines() ``` and/or
       ...
       user's buffer.
   13. `read_native_deep_scanlines()` and/or `read_native_deep_tiles()`,
...
```{literalinclude} ../jpeg.imageio/jpeginput.cpp
:language: cpp
```
...
1. Read the base class definition from {file}`imageio.h`, just as
   with an image reader (see Section [Image Reader Plugins](#image-reader-plugins)).
...
2. Declare four public items:
   ...
   2. A function named `name_output_imageio_create` that takes no
      ...
      *name* is the name of your format, and must match the name of the
      plugin itself.)
   3. An array of `char *` called `name_output_extensions`
      that contains the list of file extensions that are likely to indicate
      a file of the right format. The list is terminated by a `nullptr`
      pointer.
...
3. The definition and implementation of an ImageOutput subclass for
   ...
   1. `format_name()` should return the name of the format, which ought
      to match the name of the plugin and by convention is strictly
      lower-case and contains no whitespace.
   2. `supports()` should return `true` if its argument names a feature
   ...
   3. `open()` should open the file and return true, or should return
   ...
   5. `write_scanline()` should write a single scanline to the file,
      translating from internal to native data format and handling strides
      properly.
   6. The virtual destructor, which should `close()` if the file is
   ...
   07. `write_scanlines()`, only if your format supports writing scanlines
   ...
   08. `write_tile()`, only if your format supports writing tiled images.
   09. `write_tiles()`, only if your format supports writing tiled images
       and you can get a performance improvement when outputting multiple
       tiles at once. If you don't supply `write_tiles()`, the default
       implementation will simply call `write_tile()` separately for each
       tile in the range.
   10. `write_rectangle()`, only if your format supports
       writing arbitrary rectangles.
   11. `write_image()`, only if you have a more clever method of doing so
       than the default implementation that calls `write_scanline()` or
       `write_tile()` repeatedly.
   12. `write_deep_scanlines()` and/or `write_deep_tiles()`, only if
...
```{literalinclude} ../jpeg.imageio/jpegoutput.cpp
:language: cpp
```
