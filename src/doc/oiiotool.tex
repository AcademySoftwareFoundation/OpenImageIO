...
Increase the contrast of the image by 50\% (1.5x) by enhancing the
difference between each pixel's value and 0.5 (the \emph{pivot}).

\begin{code}
    oiiotool original.exr --contrast:pivot=0.5,0.5,0.5 1.5,1.5,1.5 -o out.exr
\end{code}
...
\apiitem{--contrast \emph{scale0,scale1,scale2...}}
\NEW
Apply an adjustment to image contrast on a pixel-by-pixel and
channel-by-channel basis:
\begin{code}
    result[c] = (input[c] - pivot[c]) * scale[c] + pivot[c]
\end{code}
Scale 
...
\begin{tabular}{p{10pt} p{1in} p{3.5in}}
  & {\cf pivot=}\emph{p0,p1,...} & Specify the pivot value per
                                   channel (default is 0.5) \\
  & {\cf lum=1} & Adjust the pixels to apply the scale to their
      luminance. In this case, only one pivot and scale value are
      needed. \\
  & {\cf clamp=0} & The default is to clamp output pixels to have a
                    minimum value of 0, but if {\cf clamp=0} is among
                    the options, no clamp will be applied. \\
\end{tabular}

\noindent Examples:

\begin{code}
    oiiotool in.tif --contrast 0.75,1,1 -o out.tif
    oiiotool in.tif --contrast:pivot=0.5,0.35,0.41 1.2,1.2,1.5 -o out.tif
    oiiotool in.tif --contrast:lum=1 1.2 -o out.tif
\end{code}
