...
\NEW % 1.8
Replace the \emph{two} top images with a new deep image that is the ``deep
holdout'' of the first image by the second --- that is, the second image
will hold out (block) samples behind them, but not themselves contribute
directly to the output image. This is the ``deep'' equivalent to a matte
holdout operation.
\apiend
...
\index{composite}
\NEW % 1.8
Replace the \emph{two} top images with a new deep image that is the first
(source) image with all samples deleted that are behind the opaque depth of
each pixel in the second (holdout) image.
\apiend
