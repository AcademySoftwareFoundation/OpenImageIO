...
\apiitem{int face}
The face or subimage within the file to use for the texture lookup.
...
This will be ignored if the file does not have separate per-face textures.
\apiend
...
\apiitem{int face}
\vspace{10pt}
The face (subimage) within the file.
This will be ignored if the file does not have separate per-face textures.
\apiend
...
\vspace{-24pt}
\apiitem{VaryingRef<float> swidth, twidth, rwidth}
\vspace{10pt}
For each direction, gives a multiplier for the derivatives.
\apiend

\vspace{-24pt}
\apiitem{VaryingRef<float> sblur, tblur, rblur}
\vspace{10pt}
For each direction, specifies an additional amount of pre-blur to apply
to the texture (\emph{after} derivatives are taken into account),
expressed as a portion of the width of the texture.
...
\apiitem{bool {\ce get_texels} (ustring filename, TextureOptions \&options, int level, \\
...
Note that the face/subimage is communicated through {\kw options.face}.
