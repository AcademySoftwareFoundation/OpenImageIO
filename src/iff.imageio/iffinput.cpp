...
IffInput::open(const std::string& name, ImageSpec& spec)
{
    ...
    // are > 64k pixels per side. The compression-ratio guard rejects a tiny
    // file that declares a huge image before the caller allocates the full
    // image buffer (readimg() has a finer per-tile plausibility check, but it
    // only runs once pixels are requested).
