...
dist_includes	:= dassert.h export.h filesystem.h hash.h \
...
#shared LINK_ILMBASE ?= ${LD_LIBPATH}${ILMBASE_LIB_AREA} ${LINKWITH}Imath ${LINK_ILMBASE_HALF} ${LINKWITH}IlmThread ${LINKWITH}Iex
...
GTEST_VERSION ?= 1.3.0
GTEST_HOME ?= ${THIRD_PARTY_TOOLS_HOME}
GTEST_CXX ?= -I${GTEST_HOME}/include/gtest-${GTEST_VERSION}
GTEST_LIB_AREA ?= ${GTEST_HOME}/lib/gtest-${GTEST_VERSION}
LINK_GTEST ?= ${LD_LIBPATH}${GTEST_LIB_AREA} ${LINKWITH}gtest

GLEW_VERSION ?= 1.5.1
GLEW_HOME ?= ${THIRD_PARTY_TOOLS_HOME}
GLEW_CXX ?= -I${GLEW_HOME}/include/glew-${GLEW_VERSION}/GL
GLEW_LIB_AREA ?= ${GLEW_HOME}/lib/glew-${GLEW_VERSION}
LINK_GLEW ?= ${GLEW_LIB_AREA}/${LIBPREFIX}GLEW${LIBEXT}
