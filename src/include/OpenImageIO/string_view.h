...
public:
    ...
    OIIO_DEPRECATED(
        "Unsafe, nonstandard. Use standalone OIIO::c_str(string_view) if you must. (3.0)")
    ...
    {
        return (m_chars ? string(m_chars, m_len)
    ...
    constexpr
    operator std::experimental::basic_string_view<CharT, Traits>() const noexcept
    {
        return { data(), size() };
    }
#endif
    ...
    constexpr int compare(basic_string_view x) const noexcept
    {
        ...
        const int cmp = traits_type::compare(m_chars, x.m_chars,
                                             (std::min)(m_len, x.m_len));
    ...
    size_type rfind(basic_string_view s, size_t pos = npos) const noexcept
    {
        ...
        const_reverse_iterator i = std::search(b, e, s.crbegin(), s.crend(),
                                               traits::eq);
        return i == e ? npos
                      : (reverse_distance(this->crbegin(), i) - s.size() + 1);
    }
    ...
    size_type find_first_of(basic_string_view s, size_t pos = 0) const noexcept
    {
        if (pos >= size())
            return npos;
        const_iterator i = std::find_first_of(this->cbegin() + pos,
                                              this->cend(), s.cbegin(),
                                              s.cend(), traits::eq);
        return i == this->cend() ? npos : std::distance(this->cbegin(), i);
    }
                           ...
                           size_t pos = npos) const noexcept
    {
        if (pos > size())
            pos = size();
        size_t off               = size() - pos;
        const_reverse_iterator i = std::find_first_of(this->crbegin() + off,
                                                      this->crend(), s.cbegin(),
                                                      s.cend(), traits::eq);
                               ...
                               size_t pos = npos) const noexcept
    {
        if (pos > size())
            pos = size();
        size_t off               = size() - pos;
        const_reverse_iterator i = find_not_of(this->crbegin() + off,
                                               this->crend(), s);
        return i == this->crend() ? npos : reverse_distance(this->crbegin(), i);
    }
    ...
    size_type find_last_not_of(CharT c, size_t pos = npos) const noexcept
    {
        if (pos > size())
            pos = size();
        size_t off = size() - pos;
        for (const_reverse_iterator i = this->crbegin() + off;
             i != this->crend(); ++i)
            if (!traits::eq(c, *i))
...
template<class CharT, class Traits = std::char_traits<CharT>>
constexpr ptrdiff_t
ssize(const OIIO::basic_string_view<CharT, Traits>& c)
{
