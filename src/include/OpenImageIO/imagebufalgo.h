...
public:
    ...
    Image_or_Const(const float (&array)[N])
        : Image_or_Const(cspan<float>(array))
    {
    }
...
ImageBuf OIIO_API fill(cspan<float> top, cspan<float> bottom, ROI roi,
                       int nthreads = 0);
ImageBuf OIIO_API fill(cspan<float> topleft, cspan<float> topright,
                       cspan<float> bottomleft, cspan<float> bottomright,
                       ROI roi, int nthreads = 0);
bool OIIO_API fill(ImageBuf& dst, cspan<float> values, ROI roi = {},
                   int nthreads = 0);
...
ImageBuf OIIO_API checker(int width, int height, int depth, cspan<float> color1,
                          cspan<float> color2, int xoffset, int yoffset,
                          int zoffset, ROI roi, int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API checker(ImageBuf& dst, int width, int height, int depth,
                      cspan<float> color1, cspan<float> color2, int xoffset = 0,
                      int yoffset = 0, int zoffset = 0, ROI roi = {},
                      int nthreads = 0);
...
/// repeatable.
ImageBuf OIIO_API noise(string_view noisetype, float A = 0.0f, float B = 0.1f,
                        bool mono = false, int seed = 0, ROI roi = {},
                        int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API noise(ImageBuf& dst, string_view noisetype, float A = 0.0f,
                    float B = 0.1f, bool mono = false, int seed = 0,
                    ROI roi = {}, int nthreads = 0);
...
bool OIIO_API render_point(ImageBuf& dst, int x, int y,
                           cspan<float> color = 1.0f, ROI roi = {},
                           int nthreads = 0);
...
bool OIIO_API render_line(ImageBuf& dst, int x1, int y1, int x2, int y2,
                          ...
                          bool skip_first_point = false, ROI roi = {},
                          int nthreads = 0);
...
///
ImageBuf OIIO_API channels(const ImageBuf& src, int nchannels,
                           ...
                           int nthreads                       = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API channels(ImageBuf& dst, const ImageBuf& src, int nchannels,
...
bool OIIO_API copy(ImageBuf& dst, const ImageBuf& src,
                   TypeDesc convert = TypeUnknown, ROI roi = {},
                   int nthreads = 0);
...
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API crop(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                   int nthreads = 0);
...
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API cut(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                  int nthreads = 0);
...
bool OIIO_API paste(ImageBuf& dst, int xbegin, int ybegin, int zbegin,
                    int chbegin, const ImageBuf& src, ROI srcroi = {},
                    int nthreads = 0);
...
ImageBuf OIIO_API rotate180(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
ImageBuf OIIO_API rotate270(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
bool OIIO_API rotate90(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                       int nthreads = 0);
bool OIIO_API rotate180(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
bool OIIO_API rotate270(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
/// @}
...
ImageBuf OIIO_API transpose(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
bool OIIO_API flip(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                   int nthreads = 0);
bool OIIO_API flop(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                   int nthreads = 0);
bool OIIO_API transpose(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
/// @}
...
/// [4,5,0,1,2,3].
ImageBuf OIIO_API circular_shift(const ImageBuf& src, int xshift, int yshift,
                                 int zshift = 0, ROI roi = {},
                                 int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API circular_shift(ImageBuf& dst, const ImageBuf& src, int xshift,
                             int yshift, int zshift = 0, ROI roi = {},
                             int nthreads = 0);
...
ImageBuf OIIO_API rotate(const ImageBuf& src, float angle,
                         ...
                         ROI roi = {}, int nthreads = 0);
ImageBuf OIIO_API rotate(const ImageBuf& src, float angle, Filter2D* filter,
                         bool recompute_roi = false, ROI roi = {},
                         int nthreads = 0);
ImageBuf OIIO_API rotate(const ImageBuf& src, float angle, float center_x,
                         float center_y, string_view filtername = string_view(),
                         float filterwidth = 0.0f, bool recompute_roi = false,
                         ROI roi = {}, int nthreads = 0);
ImageBuf OIIO_API rotate(const ImageBuf& src, float angle, float center_x,
                         float center_y, Filter2D* filter,
                         bool recompute_roi = false, ROI roi = {},
                         int nthreads = 0);
bool OIIO_API rotate(ImageBuf& dst, const ImageBuf& src, float angle,
                     ...
                     ROI roi = {}, int nthreads = 0);
bool OIIO_API rotate(ImageBuf& dst, const ImageBuf& src, float angle,
                     Filter2D* filter, bool recompute_roi = false, ROI roi = {},
                     int nthreads = 0);
bool OIIO_API rotate(ImageBuf& dst, const ImageBuf& src, float angle,
...
ImageBuf OIIO_API resize(const ImageBuf& src, KWArgs options = {}, ROI roi = {},
                         int nthreads = 0);
...
ImageBuf OIIO_API fit(const ImageBuf& src, KWArgs options = {}, ROI roi = {},
                      int nthreads = 0);
...
ImageBuf OIIO_API warp(const ImageBuf& src, M33fParam M, KWArgs options = {},
                       ROI roi = {}, int nthreads = 0);
...
ImageBuf OIIO_API st_warp(const ImageBuf& src, const ImageBuf& stbuf,
                          string_view filtername = string_view(),
                          float filterwidth = 0.0f, int chan_s = 0,
                          int chan_t = 1, bool flip_s = false,
                          bool flip_t = false, ROI roi = {}, int nthreads = 0);
ImageBuf OIIO_API st_warp(const ImageBuf& src, const ImageBuf& stbuf,
                          const Filter2D* filter, int chan_s = 0,
                          int chan_t = 1, bool flip_s = false,
                          bool flip_t = false, ROI roi = {}, int nthreads = 0);
bool OIIO_API st_warp(ImageBuf& dst, const ImageBuf& src, const ImageBuf& stbuf,
                      string_view filtername = string_view(),
...
ImageBuf OIIO_API add(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// giving a per-channel constant, or a single constant used for all
/// channels. (But at least one must be an image.)
ImageBuf OIIO_API sub(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// giving a per- channel constant, or a single constant used for all
/// channels. (But at least one must be an image.)
ImageBuf OIIO_API absdiff(Image_or_Const A, Image_or_Const B, ROI roi = {},
                          int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API abs(ImageBuf& dst, const ImageBuf& A, ROI roi = {},
                  int nthreads = 0);
...
ImageBuf OIIO_API mul(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// giving a per-channel constant or a single constant used for all
/// channels.
ImageBuf OIIO_API div(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API mad(Image_or_Const A, Image_or_Const B, Image_or_Const C,
                      ROI roi = {}, int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API over(const ImageBuf& A, const ImageBuf& B, ROI roi = {},
                       int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API zover(const ImageBuf& A, const ImageBuf& B,
                        bool z_zeroisinf = false, ROI roi = {},
                        int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API invert(ImageBuf& dst, const ImageBuf& A, ROI roi = {},
                     int nthreads = 0);
...
/// giving a per-channel constant or a single constant used for all
/// channels.
ImageBuf OIIO_API pow(const ImageBuf& A, cspan<float> B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
bool OIIO_API channel_sum(ImageBuf& dst, const ImageBuf& src,
                          cspan<float> weights = 1.0f, ROI roi = {},
                          int nthreads = 0);
...
/// a `cspan<float>` giving a per-channel constant or a single constant
/// used for all channels.
ImageBuf OIIO_API max(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// a `cspan<float>` giving a per-channel constant or a single constant
/// used for all channels.
ImageBuf OIIO_API min(Image_or_Const A, Image_or_Const B, ROI roi = {},
                      int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API clamp(const ImageBuf& src,
                        ...
                        bool clampalpha01 = false, ROI roi = {},
                        int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
bool OIIO_API maxchan(ImageBuf& dst, const ImageBuf& A, ROI roi = {},
                      int nthreads = 0);

ImageBuf OIIO_API minchan(const ImageBuf& src, ROI roi = {}, int nthreads = 0);
bool OIIO_API minchan(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                      int nthreads = 0);
/// @}
...
OIIO_API ImageBuf contrast_remap(const ImageBuf& src, cspan<float> black = 0.0f,
                                 cspan<float> white     = 1.0f,
                                 cspan<float> min       = 0.0f,
                                 cspan<float> max       = 1.0f,
                                 cspan<float> scontrast = 1.0f,
                                 cspan<float> sthresh = 0.5f, ROI = {},
                                 int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
OIIO_API bool
contrast_remap(ImageBuf& dst, const ImageBuf& src, cspan<float> black = 0.0f,
               cspan<float> white = 1.0f, cspan<float> min = 0.0f,
               cspan<float> max = 1.0f, cspan<float> scontrast = 1.0f,
               cspan<float> sthresh = 0.5f, ROI = {}, int nthreads = 0);
...
ImageBuf OIIO_API color_map(const ImageBuf& src, int srcchannel, int nknots,
                            int channels, cspan<float> knots, ROI roi = {},
                            int nthreads = 0);
ImageBuf OIIO_API color_map(const ImageBuf& src, int srcchannel,
                            string_view mapname, ROI roi = {},
                            int nthreads = 0);
...
PixelStats OIIO_API computePixelStats(const ImageBuf& src, ROI roi = {},
                                      int nthreads = 0);
...
int OIIO_API compare_Yee(const ImageBuf& A, const ImageBuf& B,
                         CompareResults& result, float luminance = 100,
                         float fov = 45, ROI roi = {}, int nthreads = 0);
...
OIIO_NODISCARD OIIO_API ImageBuf FLIP_diff(const ImageBuf& ref,
                                           const ImageBuf& test,
                                           KWArgs options = {}, ROI roi = {},
                                           int nthreads = 0);

OIIO_NODISCARD_ERROR OIIO_API bool FLIP_diff(ImageBuf& dst, const ImageBuf& ref,
                                             const ImageBuf& test,
                                             KWArgs options = {}, ROI roi = {},
                                             int nthreads = 0);
...
FLIP_ppd(float monitor_distance_m = 0.7f, float screen_width_px = 3840.0f,
...
OIIO_API bool isConstantChannel(const ImageBuf& src, int channel, float val,
                                float threshold = 0.0f, ROI roi = {},
                                int nthreads = 0);
...
bool OIIO_API color_count(const ImageBuf& src, imagesize_t* count, int ncolors,
                          cspan<float> color, cspan<float> eps = 0.001f,
                          ROI roi = {}, int nthreads = 0);
...
/// stored.
bool OIIO_API color_range_check(const ImageBuf& src, imagesize_t* lowcount,
                                imagesize_t* highcount,
                                imagesize_t* inrangecount, cspan<float> low,
                                cspan<float> high, ROI roi = {},
                                int nthreads = 0);
...
OIIO_API ROI nonzero_region(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
...
std::vector<imagesize_t> histogram(const ImageBuf& src, int channel = 0,
                                   int bins = 256, float min = 0.0f,
                                   float max = 1.0f, bool ignore_empty = false,
                                   ROI roi = {}, int nthreads = 0);
...
///                     [ 0  1  0 ]
///
ImageBuf OIIO_API laplacian(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API laplacian(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
...
bool OIIO_API fft(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                  int nthreads = 0);
bool OIIO_API ifft(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                   int nthreads = 0);
/// @}
...
ImageBuf OIIO_API complex_to_polar(const ImageBuf& src, ROI roi = {},
                                   int nthreads = 0);
bool OIIO_API complex_to_polar(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                               int nthreads = 0);
ImageBuf OIIO_API polar_to_complex(const ImageBuf& src, ROI roi = {},
                                   int nthreads = 0);
bool OIIO_API polar_to_complex(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                               int nthreads = 0);
/// @}
...
ImageBuf OIIO_API fixNonFinite(const ImageBuf& src,
                               ...
                               int* pixelsFixed = nullptr, ROI roi = {},
                               int nthreads = 0);
...
ImageBuf OIIO_API fillholes_pushpull(const ImageBuf& src, ROI roi = {},
                                     int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API median_filter(const ImageBuf& src, int width = 3,
                                int height = -1, ROI roi = {},
                                int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API median_filter(ImageBuf& dst, const ImageBuf& src, int width = 3,
...
ImageBuf OIIO_API dilate(const ImageBuf& src, int width = 3, int height = -1,
                         ROI roi = {}, int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API dilate(ImageBuf& dst, const ImageBuf& src, int width = 3,
                     int height = -1, ROI roi = {}, int nthreads = 0);
...
ImageBuf OIIO_API erode(const ImageBuf& src, int width = 3, int height = -1,
                        ROI roi = {}, int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API erode(ImageBuf& dst, const ImageBuf& src, int width = 3,
                    int height = -1, ROI roi = {}, int nthreads = 0);
...
bool OIIO_API colorconvert(ImageBuf& dst, const ImageBuf& src,
                           ...
                           bool unpremult = true, string_view context_key = "",
                           string_view context_value      = "",
                           const ColorConfig* colorconfig = nullptr,
                           ROI roi = {}, int nthreads = 0);
...
bool OIIO_API colorconvert(span<float> color, const ColorProcessor* processor,
                           bool unpremult);
...
/// @version 2.1+
///
ImageBuf OIIO_API colormatrixtransform(const ImageBuf& src, M44fParam M,
                                       bool unpremult = true, ROI roi = {},
                                       int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
bool OIIO_API ociolook(ImageBuf& dst, const ImageBuf& src, string_view looks,
                       ...
                       string_view context_key        = "",
                       string_view context_value      = "",
                       const ColorConfig* colorconfig = nullptr, ROI roi = {},
                       int nthreads = 0);
...
///             color configuration found by examining the `$OCIO`
///             environment variable will be used instead.
ImageBuf OIIO_API ociodisplay(const ImageBuf& src, string_view display,
...
///             color configuration found by examining the `$OCIO`
///             environment variable will be used instead.
ImageBuf OIIO_API ociofiletransform(const ImageBuf& src, string_view name,
                                    bool unpremult = true, bool inverse = false,
                                    const ColorConfig* colorconfig = nullptr,
                                    ROI roi = {}, int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
ImageBuf OIIO_API unpremult(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
bool OIIO_API unpremult(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
ImageBuf OIIO_API premult(const ImageBuf& src, ROI roi = {}, int nthreads = 0);
bool OIIO_API premult(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                      int nthreads = 0);
ImageBuf OIIO_API repremult(const ImageBuf& src, ROI roi = {},
                            int nthreads = 0);
bool OIIO_API repremult(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                        int nthreads = 0);
/// @}
...
bool OIIO_API make_texture(MakeTextureMode mode, const ImageBuf& input,
                           string_view outputfilename, const ImageSpec& config,
...
bool OIIO_API make_texture(MakeTextureMode mode, string_view filename,
                           string_view outputfilename, const ImageSpec& config,
                           std::ostream* outstream = nullptr);
...
/// `src`.
ImageBuf OIIO_API deepen(const ImageBuf& src, float zvalue = 1.0f, ROI roi = {},
                         int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
...
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API flatten(ImageBuf& dst, const ImageBuf& src, ROI roi = {},
                      int nthreads = 0);
...
ImageBuf OIIO_API deep_merge(const ImageBuf& A, const ImageBuf& B,
                             bool occlusion_cull = true, ROI roi = {},
                             int nthreads = 0);
/// Write to an existing image `dst` (allocating if it is uninitialized).
bool OIIO_API deep_merge(ImageBuf& dst, const ImageBuf& A, const ImageBuf& B,
                         bool occlusion_cull = true, ROI roi = {},
                         int nthreads = 0);
...
bool OIIO_API deep_holdout(ImageBuf& dst, const ImageBuf& src,
                           const ImageBuf& holdout, ROI roi = {},
                           int nthreads = 0);
...
ociodisplay(const ImageBuf& src, string_view display, string_view view,
            string_view fromspace, string_view looks, bool unpremult,
...
ociodisplay(ImageBuf& dst, const ImageBuf& src, string_view display,
            string_view view, string_view fromspace, string_view looks,
            bool unpremult, string_view context_key,
            string_view context_value      = "",
            const ColorConfig* colorconfig = nullptr, ROI roi = {},
            int nthreads = 0)
{
    return ociodisplay(dst, src, display, view, fromspace, looks, unpremult,
                       false, context_key, context_value, colorconfig, roi,
                       nthreads);
}
...
ImageBuf OIIO_API warp(const ImageBuf& src, M33fParam M, string_view filtername,
                       float filterwidth = 0.0f, bool recompute_roi = false,
                       ImageBuf::WrapMode wrap = ImageBuf::WrapDefault,
                       ROI roi = {}, int nthreads = 0);
ImageBuf OIIO_API warp(const ImageBuf& src, M33fParam M, const Filter2D* filter,
...
inline ImageBuf
resize(const ImageBuf& src, string_view filtername, float filterwidth = 0.0f,
       ROI roi = {}, int nthreads = 0)
{
    return resize(src,
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline bool
resize(ImageBuf& dst, const ImageBuf& src, string_view filtername,
       float filterwidth, ROI roi = {}, int nthreads = 0)
{
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline bool
resize(ImageBuf& dst, const ImageBuf& src, Filter2D* filter, ROI roi = {},
       int nthreads = 0)
{
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline ImageBuf
fit(const ImageBuf& src, string_view filtername = "", float filterwidth = 0.0f,
    string_view fillmode = "letterbox", bool exact = false, ROI roi = {},
    int nthreads = 0)
{
    return fit(src,
               { { "filtername", filtername },
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline ImageBuf
fit(const ImageBuf& src, Filter2D* filter, string_view fillmode = "letterbox",
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline bool
fit(ImageBuf& dst, const ImageBuf& src, string_view filtername = "",
    float filterwidth = 0.0f, string_view fillmode = "letterbox",
    bool exact = false, ROI roi = {}, int nthreads = 0)
{
    return fit(dst, src,
               { { "filtername", filtername },
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline bool
fit(ImageBuf& dst, const ImageBuf& src, Filter2D* filter,
    string_view fillmode = "letterbox", bool exact = false, ROI roi = {},
    int nthreads = 0)
{
    return fit(dst, src,
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline ImageBuf
fit(const ImageBuf& src, string_view filtername, float filterwidth, bool exact,
    ROI roi = {}, int nthreads = 0)
{
    return fit(src,
               { { "filtername", filtername },
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline ImageBuf
fit(const ImageBuf& src, Filter2D* filter, bool exact, ROI roi = {},
    int nthreads = 0)
{
    return fit(src,
               { make_pv("filterptr", filter),
...
inline bool
fit(ImageBuf& dst, const ImageBuf& src, string_view filtername,
    float filterwidth, bool exact, ROI roi = {}, int nthreads = 0)
{
    return fit(dst, src,
               { { "filtername", filtername },
                 { "filterwidth", filterwidth },
...
OIIO_DEPRECATED("prefer the kind that takes keyword args (3.0)")
inline bool
fit(ImageBuf& dst, const ImageBuf& src, Filter2D* filter, bool exact,
    ROI roi = {}, int nthreads = 0)
{
    return fit(dst, src,
               { make_pv("filterptr", filter),
