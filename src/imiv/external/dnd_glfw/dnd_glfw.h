...
enum class PayloadKind { Unknown = 0, Files = 1 };
...
struct Callbacks {
    void (*dragEnter)(GLFWwindow* window, const DragEvent& event,
                      void* userData)
        = nullptr;
    void (*dragOver)(GLFWwindow* window, const DragEvent& event, void* userData)
        = nullptr;
    void (*dragLeave)(GLFWwindow* window, void* userData) = nullptr;
    void (*drop)(GLFWwindow* window, const DropEvent& event, void* userData)
        = nullptr;
...
namespace detail {
    ...
    inline std::vector<WindowState>& windowStates()
    {
        static std::vector<WindowState> states;
        return states;
    }

    inline WindowState* findState(GLFWwindow* window)
    {
    ...
    inline WindowState* addState(GLFWwindow* window)
    {
        auto& states = windowStates();
    ...
    inline void removeState(GLFWwindow* window)
    {
        auto& states = windowStates();
    ...
    inline void dispatchDragEnter(WindowState* state, double x, double y,
                                  PayloadKind kind)
    {
    ...
    inline void dispatchDragOver(WindowState* state, double x, double y,
                                 PayloadKind kind)
    {
    ...
    inline void dispatchDragLeave(WindowState* state)
    {
    ...
    inline void dispatchDragCancel(WindowState* state)
    {
    ...
    inline void dispatchDrop(WindowState* state, const DropEvent& ev)
    {
    ...
    inline void glfwDropCallback(GLFWwindow* window, int count,
                                 const char** paths);
    ...
    struct DropTarget : public IDropTarget {
        ...
        HRESULT STDMETHODCALLTYPE DragEnter(IDataObject* dataObj,
        ...
        HRESULT STDMETHODCALLTYPE Drop(IDataObject* dataObj, DWORD grfKeyState,
                                       POINTL pt, DWORD* pdwEffect) override
        {
            ...
            for (UINT i = 0; i < fileCount; ++i) {
                ...
                int utf8Len = WideCharToMultiByte(CP_UTF8, 0, wpath.c_str(),
                ...
                WideCharToMultiByte(CP_UTF8, 0, wpath.c_str(),
                                    static_cast<int>(wpath.size()), &path[0],
