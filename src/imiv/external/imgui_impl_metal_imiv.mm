...
- (instancetype)initWithTexture:(id<MTLTexture>)metalTexture samplerState:(id<MTLSamplerState>)samplerState;
@end
...
ImTextureID ImGui_ImplMetal_CreateUserTextureID(id<MTLTexture> texture,
...
void ImGui_ImplMetal_DestroyUserTextureID(ImTextureID tex_id)
{
    if (tex_id == ImTextureID_Invalid)
        return;
    MetalTexture* backend_tex = (__bridge_transfer MetalTexture*)(void*)(intptr_t)(tex_id);
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
                id<MTLSamplerState> sampler_state = bd->SharedMetalContext.defaultSamplerState;
                if (ImTextureID tex_id = pcmd->GetTexID())
                {
                    ...
                    if ([metal_object isKindOfClass:[MetalTexture class]])
                    {
                        ...
                        [commandEncoder setFragmentTexture:backend_tex.metalTexture atIndex:0];
                        if (backend_tex.samplerState != nil)
                            sampler_state = backend_tex.samplerState;
                    }
                    else
                    {
                        [commandEncoder setFragmentTexture:(id<MTLTexture>)metal_object atIndex:0];
                    }
                }
                [commandEncoder setFragmentSamplerState:sampler_state atIndex:0];
...
bool ImGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
    ...
    bd->SharedMetalContext.defaultSamplerState = [device newSamplerStateWithDescriptor:samplerDescriptor];
