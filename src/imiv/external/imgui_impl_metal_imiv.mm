...
- (instancetype)initWithTexture:(id<MTLTexture>)metalTexture
                   samplerState:(id<MTLSamplerState>)samplerState;
@end
                                    ...
                                    id<MTLSamplerState> sampler_state)
{
    ...
    MetalTexture* backend_tex = [[MetalTexture alloc]
        initWithTexture:texture
           samplerState:sampler_state];
    return (ImTextureID)(__bridge_retained void*)(backend_tex);
}
...
ImGui_ImplMetal_DestroyUserTextureID(ImTextureID tex_id)
{
    ...
    MetalTexture* backend_tex
        = (__bridge_transfer MetalTexture*)(void*)(intptr_t)(tex_id);
    backend_tex.metalTexture = nil;
    backend_tex.samplerState = nil;
}

static void
ImGui_ImplMetal_SetupRenderState(ImDrawData* draw_data,
                                 ...
                                 size_t vertexBufferOffset)
{
    ...
    MTLViewport viewport = { .originX = 0.0,
                             .originY = 0.0,
                             .width   = (double)(draw_data->DisplaySize.x
                                               * draw_data->FramebufferScale.x),
                             .height  = (double)(draw_data->DisplaySize.y
                               ...
                               id<MTLRenderCommandEncoder> commandEncoder)
{
    ...
    size_t vertexBufferLength = (size_t)draw_data->TotalVtxCount
                                * sizeof(ImDrawVert);
    size_t indexBufferLength = (size_t)draw_data->TotalIdxCount
                               * sizeof(ImDrawIdx);
    MetalBuffer* vertexBuffer = [ctx
        dequeueReusableBufferOfLength:vertexBufferLength
                               device:commandBuffer.device];
    MetalBuffer* indexBuffer  = [ctx
        dequeueReusableBufferOfLength:indexBufferLength
                               device:commandBuffer.device];

    ImGui_ImplMetal_SetupRenderState(draw_data, commandBuffer, commandEncoder,
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists) {
            ...
            if (pcmd->UserCallback) {
                ...
                if (tex_id != ImTextureID_Invalid) {
                    ...
                    if ([metal_object isKindOfClass:[MetalTexture class]]) {
                        ...
                        [commandEncoder
                            setFragmentTexture:backend_tex.metalTexture
                                       atIndex:0];
                        if (backend_tex.samplerState != nil)
                            sampler_state = backend_tex.samplerState;
                    } else {
                        [commandEncoder
                            setFragmentTexture:(id<MTLTexture>)metal_object
                                       atIndex:0];
                    }
                }
                [commandEncoder setFragmentSamplerState:sampler_state
                                                atIndex:0];
        ...
        vertexBufferOffset += (size_t)draw_list->VtxBuffer.Size
                              * sizeof(ImDrawVert);
        indexBufferOffset += (size_t)draw_list->IdxBuffer.Size
                             * sizeof(ImDrawIdx);
    }
...
static void
ImGui_ImplMetal_DestroyTexture(ImTextureData* tex)
{
    if (MetalTexture* backend_tex
        = (__bridge_transfer MetalTexture*)(tex->BackendUserData)) {
        IM_ASSERT(backend_tex.metalTexture
                  == (__bridge id<MTLTexture>)(void*)(intptr_t)tex->TexID);
        backend_tex.metalTexture = nil;
...
void
ImGui_ImplMetal_UpdateTexture(ImTextureData* tex)
{
    ImGui_ImplMetal_Data* bd = ImGui_ImplMetal_GetBackendData();
...
#    endif
        id<MTLTexture> texture = [bd->SharedMetalContext.device
            newTextureWithDescriptor:textureDescriptor];
        [texture replaceRegion:MTLRegionMake2D(0, 0, (NSUInteger)tex->Width,
        ...
        MetalTexture* backend_tex
            = (__bridge MetalTexture*)(tex->BackendUserData);
        for (ImTextureRect& r : tex->Updates) {
            [backend_tex.metalTexture
                replaceRegion:MTLRegionMake2D((NSUInteger)r.x, (NSUInteger)r.y,
                                              (NSUInteger)r.w, (NSUInteger)r.h)
                  mipmapLevel:0
...
ImGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
    ...
    bd->SharedMetalContext.depthStencilState    = [device
...
static void
ImGui_ImplMetal_CreateWindow(ImGuiViewport* viewport)
{
    ImGui_ImplMetal_Data* bd     = ImGui_ImplMetal_GetBackendData();
...
static void
ImGui_ImplMetal_DestroyWindow(ImGuiViewport* viewport)
{
    // The main viewport (owned by the application) will always have RendererUserData == 0 since we didn't create the data for it.
    if (ImGuiViewportDataMetal* data
        = (ImGuiViewportDataMetal*)viewport->RendererUserData)
        IM_DELETE(data);
    viewport->RendererUserData = nullptr;
}

inline static CGSize
MakeScaledSize(CGSize size, CGFloat scale)
{
    return CGSizeMake(size.width * scale, size.height * scale);
}

static void
ImGui_ImplMetal_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ImGuiViewportDataMetal* data
        = (ImGuiViewportDataMetal*)viewport->RendererUserData;
    data->MetalLayer.drawableSize = MakeScaledSize(CGSizeMake(size.x, size.y),
                                                   viewport->DpiScale);
}

static void
ImGui_ImplMetal_RenderWindow(ImGuiViewport* viewport, void*)
{
    ImGuiViewportDataMetal* data
        = (ImGuiViewportDataMetal*)viewport->RendererUserData;
...
#    if TARGET_OS_OSX
    ...
    if (data->MetalLayer.contentsScale != fb_scale) {
        ...
        data->MetalLayer.drawableSize  = MakeScaledSize(window.frame.size,
                                                        fb_scale);
    }
#    endif
    ...
    ImGui_ImplMetal_RenderDrawData(viewport->DrawData, commandBuffer,
                                   renderEncoder);
...
{
    FramebufferDescriptor* copy = [[FramebufferDescriptor allocWithZone:zone]
        init];
...
- (instancetype)initWithTexture:(id<MTLTexture>)metalTexture
{
    return [self initWithTexture:metalTexture samplerState:nil];
}
                                         ...
                                         device:(id<MTLDevice>)device
{
    ...
    pipelineDescriptor.rasterSampleCount
        = self.framebufferDescriptor.sampleCount;
