#!/usr/bin/env python3
"""Basic screenshot smoke test for the Metal imiv backend."""
...
def main() -> int:
    ...
    out_dir = (
        ...
        if args.out_dir
        else exe.parent.parent / "imiv_captures" / "metal_screenshot_regression"
    )
    ...
    screenshot_path = out_dir / "metal_smoke.png"
    layout_path = out_dir / "metal_smoke.layout.json"
    state_path = out_dir / "metal_smoke.state.json"
    log_path = out_dir / "metal_smoke.log"
    ...
    if screenshot_path.stat().st_size <= 0:
        print(f"error: screenshot file is empty: {screenshot_path}", file=sys.stderr)
        return 1
    ...
    print(f"ok: Metal screenshot regression outputs are in {out_dir}")
    return 0
