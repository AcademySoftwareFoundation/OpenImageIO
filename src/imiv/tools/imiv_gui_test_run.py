...
def main() -> int:
    ...
    ap.add_argument(
        "--screenshot-out",
        default="",
        help="Enable screenshot test and write to this output path",
    )
    ap.add_argument(
        "--screenshot-frames", type=int, default=1, help="Number of screenshot frames"
    )
    ap.add_argument(
        "--screenshot-delay-frames",
        type=int,
        default=3,
        help="Initial delay before screenshot capture",
    )
    ap.add_argument(
        "--screenshot-save-all", action="store_true", help="Save all screenshot frames"
    )

    ap.add_argument(
        "--layout-json-out",
        default="",
        help="Enable layout JSON dump and write to this path",
    )
    ap.add_argument(
        "--layout-items", action="store_true", help="Include per-item data in layout JSON"
    )
    ap.add_argument("--layout-depth", type=int, default=8, help="Layout gather depth")
    ap.add_argument(
        "--layout-delay-frames", type=int, default=3, help="Initial delay before layout dump"
    )
    ap.add_argument(
        "--state-json-out",
        default="",
        help="Enable viewer state JSON dump and write to this path",
    )
    ap.add_argument(
        "--state-delay-frames",
        type=int,
        default=3,
        help="Initial delay before viewer state dump",
    )
    ap.add_argument(
        "--post-action-delay-frames",
        type=int,
        default=0,
        help="Extra frames to wait after synthetic input before capture/dump",
    )

    ap.add_argument("--svg-out", default="", help="Post-convert layout JSON to SVG at this path")
    ap.add_argument(
        "--svg-items", action="store_true", help="Draw items in SVG (implies --layout-items)"
    )
    ...
    ap.add_argument(
        "--ocio-view", default="", help="Optional live OCIO view override for automation"
    )
    ap.add_argument(
        "--ocio-image-color-space",
        default="",
    ...
    ap.add_argument(
        "--mouse-drag-button", type=int, default=0, help="Mouse button index for --mouse-drag"
    )
    ap.add_argument(
        "--mouse-drag-hold",
        nargs=2,
    ...
    if args.mouse_pos_window_rel:
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_WINDOW_REL_X"] = str(
            args.mouse_pos_window_rel[0]
        )
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_WINDOW_REL_Y"] = str(
            args.mouse_pos_window_rel[1]
        )

    if args.mouse_pos_image_rel:
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_IMAGE_REL_X"] = str(
            args.mouse_pos_image_rel[0]
        )
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_IMAGE_REL_Y"] = str(
            args.mouse_pos_image_rel[1]
        )
    ...
    if args.mouse_drag:
        ...
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_DRAG_BUTTON"] = str(
            args.mouse_drag_button
        )
    ...
    if args.mouse_drag_hold:
        ...
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_HOLD_DRAG_BUTTON"] = str(
            args.mouse_drag_hold_button
        )
        env["IMIV_IMGUI_TEST_ENGINE_MOUSE_HOLD_FRAMES"] = str(
            max(0, args.mouse_drag_hold_frames)
        )
    if args.post_action_delay_frames > 0:
        env["IMIV_IMGUI_TEST_ENGINE_POST_ACTION_DELAY_FRAMES"] = str(
            max(0, args.post_action_delay_frames)
        )
    ...
    if want_screenshot:
        ...
        env["IMIV_IMGUI_TEST_ENGINE_AUTOSSCREENSHOT_DELAY_FRAMES"] = str(
            max(0, args.screenshot_delay_frames)
        )
    ...
    if want_layout and not same_test_hold_capture:
        ...
        env["IMIV_IMGUI_TEST_ENGINE_LAYOUT_DUMP_DELAY_FRAMES"] = str(
    ...
    if want_junit:
        ...
        try:
            ...
            print(
                f"error: failed to parse junit xml '{junit_path}': {exc}",
                file=sys.stderr,
            )
            return 2
        ...
        if failures > 0 or errors > 0:
            print(
                f"error: junit reported failures={failures}, errors={errors}",
                file=sys.stderr,
            )
            return 1
