#!/usr/bin/env python3
"""Regression check for GUI-driven Export Selection As recipe export."""
...
def main() -> int:
    ...
    default_out_dir = repo_root / "build" / "imiv_captures" / "export_selection_regression"
    ...
    scenario_path = out_dir / "export_selection.scenario.xml"
    select_state_path = runtime_dir / "select_drag.state.json"
    recipe_state_path = runtime_dir / "adjust_recipe.state.json"
    save_state_path = runtime_dir / "export_selection.state.json"
    log_path = out_dir / "export_selection.log"
    fixture_path = out_dir / "export_selection_input.png"
    saved_path = out_dir / "exported_selection.tif"
    expected_path = out_dir / "expected_exported_selection.tif"
    ...
    if prep.returncode != 0:
        ...
        return _fail("failed to prepare export-selection fixture")
    ...
    if not recipe_state_path.exists():
        return _fail(f"recipe state output not found: {recipe_state_path}")
    if not save_state_path.exists():
        return _fail(f"export state output not found: {save_state_path}")
    if not saved_path.exists():
        return _fail(f"exported selection output not found: {saved_path}")
    ...
    _assert_close(float(recipe.get("exposure", 0.0)), 0.75, "exposure")
    _assert_close(float(recipe.get("gamma", 0.0)), 1.6, "gamma")
    _assert_close(float(recipe.get("offset", 0.0)), 0.125, "offset")
    if int(recipe.get("current_channel", 0)) != 3:
        return _fail("current_channel did not remain blue")
    if int(recipe.get("color_mode", 0)) != 2:
        return _fail("color_mode did not remain single channel")
    ...
    expected = _run(
        [
            ...
            "--mulc",
            "1.681792830507429,1.681792830507429,1.681792830507429,1.0",
            "--powc",
    ...
    if expected.returncode != 0:
        ...
        return _fail("failed to generate expected exported selection")

    diff = _run([str(idiff), "-q", "-a", str(expected_path), str(saved_path)], cwd=repo_root)
    if diff.returncode != 0:
        print(diff.stdout, end="")
        return _fail("exported selection did not match expected export")
