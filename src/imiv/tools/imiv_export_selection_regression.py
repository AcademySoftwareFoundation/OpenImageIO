...
def main() -> int:
    ...
    if prep.returncode != 0:
        ...
        return fail("failed to prepare export-selection fixture")
    ...
    if not save_state_path.exists():
        return fail(f"export state output not found: {save_state_path}")
    if not saved_path.exists():
        return fail(f"exported selection output not found: {saved_path}")
    ...
    if int(recipe.get("current_channel", 0)) != 3:
        return fail("current_channel did not remain blue")
    if int(recipe.get("color_mode", 0)) != 2:
        return fail("color_mode did not remain single channel")
    ...
    if expected.returncode != 0:
        ...
        return fail("failed to generate expected exported selection")
    ...
    if diff.returncode != 0:
        ...
        return fail("exported selection did not match expected export")
