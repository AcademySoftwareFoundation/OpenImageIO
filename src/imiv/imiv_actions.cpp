                             ...
                             std::string& error_message)
{
    ...
    const ROI selection_roi(viewer.selection_xbegin, viewer.selection_xend,
                            viewer.selection_ybegin, viewer.selection_yend, 0,
                            1, 0, image.nchannels);
...
build_view_export_rgba_image(const ImageBuf& source_image,
                             ...
                             ImageBuf& output, std::string& error_message)
{
    error_message.clear();
    ImageBuf oriented     = source_image;
    const int orientation = source_image.spec().get_int_attribute("Orientation",
                                                                  1);
    if (orientation != 1) {
    ...
    if (recipe.use_ocio) {
        ...
        if (!build_ocio_cpu_display_processor(export_ui_state, &image_metadata,
                                              recipe.exposure, recipe.gamma,
                                              processor, resolved_display,
                                              resolved_view, error_message)) {
            return false;
        }
        if (!processor) {
                               ...
                               ImageBuf& output, std::string& error_message)
{
    ...
    return build_view_export_rgba_image(source, image, recipe, ui_state, output,
                                        error_message);
}
...
save_selection_as_dialog_action(ViewerState& viewer)
{
    ...
    if (reply.result == FileDialog::Result::Okay) {
        ...
        if (save_selection_image(viewer.image, viewer, reply.path, error)) {
            ...
            viewer.status_message
                = Strutil::fmt::format("Saved selection {} ({}x{})", reply.path,
                                       width, height);
            viewer.last_error.clear();
        } else {
            viewer.last_error = Strutil::fmt::format("save failed: {}", error);
                                  ...
                                  const PlaceholderUiState& ui_state)
{
    ...
    if (reply.result == FileDialog::Result::Okay) {
        ...
        if (save_export_selection_image(viewer.image, viewer, ui_state,
            ...
            const int height = viewer.selection_yend - viewer.selection_ybegin;
            viewer.status_message
                = Strutil::fmt::format("Exported selection {} ({}x{})",
                                       reply.path, width, height);
            viewer.last_error.clear();
        } else {
