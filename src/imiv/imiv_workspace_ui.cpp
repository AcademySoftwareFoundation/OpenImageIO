...
namespace {
    ...
    enum class PendingImageListActionKind {
        None = 0,
        OpenInActiveView,
        OpenInNewView,
        CloseInActiveView,
    ...
    struct PendingImageListAction {
        PendingImageListActionKind kind = PendingImageListActionKind::None;
        std::string path;
    };
    ...
    void request_image_list_action(PendingImageListAction& pending_action,
                                   PendingImageListActionKind kind,
                                   const std::string& path)
    {
        pending_action.kind = kind;
        pending_action.path = path;
    }

    void append_image_list_item_rect(MultiViewWorkspace& workspace)
    {
        const ImVec2 item_min = ImGui::GetItemRectMin();
        const ImVec2 item_max = ImGui::GetItemRectMax();
        workspace.image_list_item_rects.emplace_back(item_min.x, item_min.y,
                                                     item_max.x, item_max.y);
    }

    void maybe_show_image_list_item_tooltip(const std::string& filename,
                                            const std::string& path,
                                            bool& showed_tooltip_this_frame,
                                            std::string& last_logged_path)
    {
        if (!ImGui::IsItemHovered() || filename == path)
            return;

        showed_tooltip_this_frame = true;
        if (debug_image_list_windows_enabled() && last_logged_path != path) {
            print("imiv: Image List tooltip {} for '{}'\n",
                  image_list_tooltips_disabled() ? "suppressed" : "requested",
                  path);
    ...
    void draw_image_list_row(MultiViewWorkspace& workspace,
                             const ImageLibraryState& library,
                             const ViewerState& active_view, size_t index,
                             PendingImageListAction& pending_action,
                             bool& showed_tooltip_this_frame,
                             std::string& last_logged_tooltip_path)
    {
        const std::string& path           = library.loaded_image_paths[index];
        const std::string normalized_path = normalize_image_list_path(path);
        ...
        const std::string item_label
            = Strutil::fmt::format("{} {}. {}{}###imiv_image_list_item_{}",
                                   active_view_image ? ">" : " ",
                                   static_cast<int>(index + 1), filename,
                                   open_badge, static_cast<int>(index));
        ...
        const float label_width = std::max(
            1.0f, row_width - (close_width + ImGui::GetStyle().ItemSpacing.x));
        ...
        register_layout_dump_synthetic_item("selectable", test_label.c_str());
        append_image_list_item_rect(workspace);
        maybe_show_image_list_item_tooltip(filename, path,
                                           showed_tooltip_this_frame,
                                           last_logged_tooltip_path);
        if (ImGui::IsItemClicked()) {
            request_image_list_action(
                pending_action, PendingImageListActionKind::OpenInActiveView,
                path);
        }
        if (ImGui::IsItemHovered()
            && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
            request_image_list_action(pending_action,
                                      PendingImageListActionKind::OpenInNewView,
                                      path);
        }
        if (ImGui::BeginPopupContextItem()) {
            if (ImGui::MenuItem("Open in active view")) {
                request_image_list_action(
                    pending_action,
                    PendingImageListActionKind::OpenInActiveView, path);
            }
            if (ImGui::MenuItem("Open in new view")) {
                request_image_list_action(
                    pending_action, PendingImageListActionKind::OpenInNewView,
                    path);
            }
            if (ImGui::MenuItem("Close in active view", nullptr, false,
                ...
                request_image_list_action(
                    pending_action,
                    PendingImageListActionKind::CloseInActiveView, path);
            }
            if (ImGui::MenuItem("Close in all views", nullptr, false,
                                open_count > 0)) {
                request_image_list_action(
                    pending_action, PendingImageListActionKind::CloseInAllViews,
                    path);
            }
            if (ImGui::MenuItem("Remove from session")) {
                request_image_list_action(
                    pending_action,
                    PendingImageListActionKind::RemoveFromSession, path);
            }
            ImGui::EndPopup();
        }
        ImGui::SameLine();
        if (ImGui::SmallButton(close_button_id.c_str())) {
            request_image_list_action(
                pending_action, PendingImageListActionKind::RemoveFromSession,
                path);
        }
    }

    void draw_image_list_items(MultiViewWorkspace& workspace,
                               const ImageLibraryState& library,
                               const ViewerState& active_view,
                               PendingImageListAction& pending_action)
    {
        ...
        if (ImGui::BeginChild("##image_list_items", ImVec2(0.0f, 0.0f), false,
            ...
            for (size_t i = 0, e = library.loaded_image_paths.size(); i < e;
                 ++i) {
                draw_image_list_row(workspace, library, active_view, i,
                                    pending_action, showed_tooltip_this_frame,
                                    s_last_logged_tooltip_path);
            }
        }
        ImGui::EndChild();
    ...
    void execute_pending_image_list_action(MultiViewWorkspace& workspace,
                                           ImageLibraryState& library,
                                           ViewerState& active_view,
                                           PlaceholderUiState& ui_state,
                                           RendererState& renderer_state,
                                           const PendingImageListAction& action)
    {
        if (action.path.empty())
            return;

        switch (action.kind) {
        case PendingImageListActionKind::OpenInActiveView:
            activate_image_list_path(workspace, library, active_view, ui_state,
                                     renderer_state, action.path);
            break;
        case PendingImageListActionKind::OpenInNewView:
            open_image_list_path_in_new_view(workspace, library, active_view,
                                             ui_state, renderer_state,
                                             action.path);
            break;
        case PendingImageListActionKind::CloseInActiveView:
            close_image_list_path_in_active_view(workspace, library,
                                                 active_view, ui_state,
                                                 renderer_state, action.path);
            break;
        case PendingImageListActionKind::CloseInAllViews:
            close_image_list_path_in_all_views(workspace, library, active_view,
                                               ui_state, renderer_state,
                                               action.path);
            break;
        case PendingImageListActionKind::RemoveFromSession:
            remove_image_list_path_from_session(workspace, library, active_view,
                                                ui_state, renderer_state,
                                                action.path);
            break;
        case PendingImageListActionKind::None: break;
        }
    }
                       ...
                       RendererState& renderer_state, bool reset_layout)
{
    ...
    PendingImageListAction pending_action;
    draw_image_list_items(workspace, library, active_view, pending_action);
    ImGui::End();

    execute_pending_image_list_action(workspace, library, active_view, ui_state,
                                      renderer_state, pending_action);
}
