...
# Python binding of OpenImageIO.color_interop_ids(): the canonical Color
# Interop Forum ids declared by OIIO's built-in interop identities registry
# (the same data as C++ ColorInteropIDs::all()). Assertions are printed as
# properties rather than as the id list itself, so this reference does not
# have to be regenerated every time the registry gains an identity.

import OpenImageIO as oiio

ids = oiio.color_interop_ids()

print("color_interop_ids: shape")
...
print("  stable across calls  =", oiio.color_interop_ids() == ids)

# Spec-mandated members that the registry must always declare.
print("color_interop_ids: known members")
...
print("color_interop_ids: agree with ColorConfig.get_color_interop_id")
