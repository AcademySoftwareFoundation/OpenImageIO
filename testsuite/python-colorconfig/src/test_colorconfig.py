...
try:
    ...
    print (f"Test buffer -- initial values: {buf.get_pixels()} ({spec['oiio:ColorSpace']})")
    buf = oiio.ImageBufAlgo.ociodisplay(buf, "", "")
    print (f"Test buffer -- ociodisplay #1: {buf.get_pixels()} ({buf.spec()['oiio:ColorSpace']})")
    buf = oiio.ImageBufAlgo.ociodisplay(buf, "", "")
    print (f"Test buffer -- ociodisplay #2: {buf.get_pixels()} ({buf.spec()['oiio:ColorSpace']})")
    buf = oiio.ImageBufAlgo.ociodisplay(buf, "", "", looks="-ACES 1.3 Reference Gamut Compression")
    print (f"Test buffer -- ociodisplay #3: {buf.get_pixels()} ({buf.spec()['oiio:ColorSpace']})")
    buf = oiio.ImageBufAlgo.ociodisplay(buf, "", "", looks="ACES 1.3 Reference Gamut Compression")
    print (f"Test buffer -- ociodisplay #4: {buf.get_pixels()} ({buf.spec()['oiio:ColorSpace']})")
    buf = oiio.ImageBufAlgo.ociodisplay(buf, "", "", looks="-ACES 1.3 Reference Gamut Compression, +ACES 1.3 Reference Gamut Compression")
    print (f"Test buffer -- ociodisplay #5: {buf.get_pixels()} ({buf.spec()['oiio:ColorSpace']})")
    print ("")
