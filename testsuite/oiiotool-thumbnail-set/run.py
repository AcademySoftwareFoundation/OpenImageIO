...
command += oiiotool (src + " --dup --resize:filter=box 50x38 --thumbnail-set"
                     + " --echo \"after thumbnail-set: {TOP.width}x{TOP.height}\""
                     + " -o out.tga")
command += info_command ("out.tga", safematch=True)

# Test that an embed/extract round trip matches a directly resized image.
command += oiiotool ("out.tga --thumbnail-get -o thumb.tif")
