...
check_reader(const char* filename, int use_core)
{
    ...
    if (!in->read_image(0, 0, 0, C, TypeDesc::UNKNOWN, full.data())) {
        std::printf("%s: read_image failed\n", label);
        return 1;
    }
    ...
    if (!ok) {
        std::printf("%s: read_native_tiles failed: %s\n", label,
                    in->geterror().c_str());
        return 1;
    }
    ...
    std::printf("%s: rect %dx%d smashed=%zu baddiffs=%d -> %s\n", label, rw, rh,
                smashed, baddiffs,
...
{
    if (argc < 2) {
        std::printf("usage: read-partial-tiles file.exr\n");
        return 2;
    }
    int err = 0;
